...
function Root(){
 ...
 return <><Suspense fallback={at.page==='sso-callback'?<SsoLoading/>:null}>{at.page==='atlas'&&<Home/>}{at.page==='library'&&<Library/>}{at.page==='structures'&&<Structures slug={at.slug}/>}{at.page==='about'&&<About open={at.slug==='mission'}/>}{at.page==='membership'&&<Membership/>}{at.page==='account'&&<Account/>}{at.page==='institutions'&&<Institutions/>}{(at.page==='sign-in'||at.page==='sign-up')&&<Auth key={at.page} mode={at.page}/>}{at.page==='sso-callback'&&<SsoCallback/>}{at.page==='privacy'&&<Privacy/>}{at.page==='terms'&&<Terms/>}</Suspense>{(at.page==='hero'||veil)&&<Hero leaving={at.page!=='hero'}/>}</>;
}
