  ...
  const animate=()=>{
   ...
   if(changed||moving||lastExtent<0){
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     const selected=selection.has(p.id);data.set([dx,dy,dz,(s.isolate?selected:visible.has(p.system)||selected)?1:0],i*4);selectedData[i*4]=selected?255:0;selectedData[i*4+1]=selected&&secondary.has(p.id)?255:0;
