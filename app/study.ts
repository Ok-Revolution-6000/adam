...
import {tierForDir,type Tier} from '../shared/tiers.ts';
...
export interface Work {id:string;author:AuthorId;number?:number;title:string;/** directory under content/ */dir:string;/** who may read it; see shared/tiers.ts */tier:Tier;/** the book the work is read in, for the reader header */book?:string;/** full edition, for credits */source:string;chapters:Chapter[]}
...
export const AUTHORS:Author[]=[ // in chronological order, as on the Library page
 ...
 {id:'avicenna',name:'Avicenna',dates:'980–1037',note:'Author of the Canon, the systematic summa of Galenic medicine that Maimonides’ contemporaries studied. The shelf holds Book I, the general principles, and Book II, the simple drugs, in Adomeh’s English from the Arabic; the other three books will follow.'},
...
/** Stamps each work with its tier from shared/tiers.ts, so declarations need not repeat it. */
export function withTiers(works:Omit<Work,'tier'>[]):Work[]{return works.map(w=>({...w,tier:tierForDir(w.dir)??'all'}));}
...
export const MAIMONIDES_WORKS:Work[]=withTiers([
