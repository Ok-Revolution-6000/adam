...
export function SiteBar({current}:{current:PageId}){
 ...
 return <header className="site-bar"><a className="site-brand" href="#/">Adomeh</a><nav aria-label="Adomeh">{NAV.map(l=><a key={l.page} href={l.href} aria-current={l.page===current?'page':undefined}>{l.label}</a>)}</nav>
