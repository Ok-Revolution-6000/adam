...
export interface SceneState {inspectorOpen?:boolean;explode:number;visible:SystemId[];selected:string[];/** Subset of `selected` drawn in the secondary highlight colour (related anatomy, not the structure under discussion). */secondary?:string[];/** Parts temporarily hidden so the reader can see where they sit. */hidden?:string[];isolate:boolean;view:View;rotate:boolean;reset:number}
